...
export class ProfileComponent implements OnInit {
  ...
  constructor(
    ...
    ngOnInit() {
      ...
      this.pictureService.executePictureUpload(this.username).subscribe(
        data => {
          console.log(data)
    ...
    share() {
      ...
      //Make a call to the Spring Boot Application to save the image
      this.httpClient.post('http://localhost:8400/media/upload/' + this.username, uploadImageData, { observe: 'response' })
        .subscribe((response) => {
          if (response.status === 200) {
            this.message = 'Image uploaded successfully';
          } else {
